...
class ReVidiaMain(QMainWindow):
    ...
    def initUI(self, reload=False):
        ...
        self.meta.setWindowFlags(self.windowFlags() & ~Qt.FramelessWindowHint) # Fixes Windows window bug QTBUG-52206
        self.meta.show()
    ...
    def mainLoop(self):
        ...
        while True:
            ...
            self.plotValues = ReVidia_win.rescaleData(plotsData, self.dataCap, self.size().height())
            self.plotSplitValues = ReVidia_win.rescaleData(splitPlotData, self.dataCap, self.size().height())
    ...
    def updatePlots(self):
        ...
        endScale = ReVidia_win.quadBezier(midPoint, endPoint, endCurve, self.plotsAmt - midPointPos, True)
    ...
    def paintFreq(self, event, painter):
        ...
        # Instead of painting freq, give a approximation of notes
            ...
            notes = ReVidia_win.assignNotes(self.freqList[:-1])
    ...
    def refitWindowForDock(self, dock):
        self.dockMod = dock
        dock.setStyleSheet(".QWidget {background-color: rgba(250, 250, 250, 255)}") # Fix/Hack for Windows transparency
