...
- By default the bar height will auto level to the data. 
  To normalize simply change the bar height manually or by toggling Design|Auto Level to off.
...
- Two profiles are include for fun. One is the "Monstercat-Lite", which is my best approximation of the Monstercat visualizer.
  And the other is "Recording" which is a balanced profile for decent audio from the Main|Reverse FFT
...
Design|Auto Level - (On by default) Auto scale the bar height to the data, manually adjusting the bar height will disable [0]*
